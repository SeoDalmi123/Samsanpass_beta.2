//---------------------------- INITIALIZE FIREBASE ----------------------------------------//  
        ...
        import { getAuth, onAuthStateChanged, signOut, updateProfile,sendEmailVerification } from "https://www.gstatic.com/firebasejs/9.15.0/firebase-auth.js";
        ...
        onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                if (!user.emailVerified) {
                    alert("Please verify your email before your next session for added security.");
        ...
        async function GetAllDataRealtime(){
            ...
            const unsubscribe = onSnapshot(q,(querySnapshot) => {
                ...
                tableBody.forEach((row, index) => {
                    const rows = row.childNodes;
                    const expireWebsite = rows[1].children[0].value;
                    const expirePassword = rows[4].children[0].children[0].children[0];
                    const popup = rows[7].children[0];
                    const infoSymbol = rows[7].children[1];
                
                    var endDate = new Date();
                
                    // Check if end date is already stored in local storage
                    const storedEndDate = localStorage.getItem(`endDate-${expireWebsite}-${index}`);
                    if (storedEndDate) {
                        endDate = new Date(storedEndDate);
                    } else {
                        endDate.setMonth(endDate.getMonth() + 3);
                        localStorage.setItem(`endDate-${expireWebsite}-${index}`, endDate.toString());
                    }
                    ...
                    var countDownInterval = setInterval(function() {
                        ...
                        if (remainingTime >= 0) {
                            ...
                            popup.innerHTML = "Change password by: " + " " + dateString + " " + year + " " + "(" + remainingTimeString + ")";
                            if (remainingTime <= 604800000) {
                                const setRowBorder = row;
                                setRowBorder.style.border = ".2em dashed #FFA500";
                                expirePassword.style.border = ".1em solid red";    
                                const setInfoColor = row.children[7].children[1];
                                setInfoColor.style.color = "#FFA500";
                                popup.style.backgroundColor = "#FF0000CA";
                            }
                        } else {
